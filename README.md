...
| `api_key` | 空 | **客户端接口**鉴权密钥（`/v1/*`、`/status`）；**空 = 不鉴权直接放行**（公网必须设置） |
| `panel_key` | 空 | **管理面板**鉴权密钥；**空 = 回落 `api_key`**（单一密钥形态，向后兼容） |
...
http://127.0.0.1:7863/panel/
```
鉴权口径：面板校验 **`panel_key`**，未配置该项时回落 `api_key`（与旧版行为一致）。非空时面板要求输入一次密钥（浏览器 localStorage 记住）；为空则直接可用。

> **服务器部署务必设置独立的 `panel_key`。** `api_key` 通常要交给客户端（Cherry Studio / ChatBox / Codex）使用；
> 若两把密钥共用，持有客户端密钥的人即可登录管理后台删号、禁用账号、读取全部账号。
> 首启自动生成的配置已默认拆成两把随机密钥；启动日志会明确提示当前是否已拆分。

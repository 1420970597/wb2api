...
func main() {
	...
	// 面板密钥状态单独透出：服务器部署时必须能一眼确认"面板密钥是否已与 API Key 拆分"，
	// 否则容易误以为已经隔离、实际仍在共用一把密钥。
	if cfg.PanelKey != "" && cfg.PanelKey != cfg.APIKey {
		log.Printf("panel_key 已独立配置：持有 api_key 的客户端无法登录管理面板")
	} else if cfg.APIKey != "" {
		log.Printf("警告：panel_key 未配置，管理面板与 /v1 共用同一把 api_key（客户端可登录面板）；" +
			"服务器部署建议在 config.json 设置独立的 panel_key")
	}

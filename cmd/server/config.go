...
type Config struct {
	...
	APIKey    string `json:"api_key"`    // 空 = 不鉴权；/v1 等客户端接口用
	PanelKey  string `json:"panel_key"`  // 管理面板独立密钥；空 = 回落 api_key（单一密钥形态）
...
// 值取自 Default()（含超时/熔断/签到排程等推荐值），api_key 与 panel_key 用
// crypto/rand 各自随机生成：安全默认优于示例占位符（listen 绑定 0.0.0.0，
// 空 key 会把网关裸暴露给局域网；两把密钥共用则客户端可登录管理面板）。
// 返回生成的 api_key 供启动日志透出。已存在时经 O_EXCL 原子拒绝，绝不改写用户配置。
func WriteDefault(path string) (string, error) {
	...
	// 面板密钥独立生成：服务器部署时 API Key 通常要交给客户端使用，
	// 若与面板共用，拿到 key 的人即可进入管理面板（删号/禁用/读全部账号）。
	// 首启即拆开，避免"默认不安全、要用户自己发现并修"。
	praw := make([]byte, 18)
	if _, err := rand.Read(praw); err != nil {
		return "", fmt.Errorf("gen panel_key: %w", err)
	}
	pkey := "pk-" + base64.RawURLEncoding.EncodeToString(praw)
	c := Default()
	c.APIKey = key
	c.PanelKey = pkey

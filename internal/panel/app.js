...
bindEye('btnEyePanel', 'cfgPanelKey');
...
$('cfgForm').onsubmit = async ev => {
  ...
  try {
    ...
    // 面板鉴权用的是 panel_key（未配置时才回落 api_key），故此处必须存 panel_key：
    // 若沿用 api_key，一旦两者不同，下一次轮询就会 401 被踢回密钥弹窗。
    const k = ($('cfgPanelKey').value || '').trim();
